-- Fix RLS policies to allow doctors to update their own appointments
-- This addresses the 400 error when doctors try to update appointment status
...
-- This is the key policy for doctor updates
...
-- Debug: Create a function to test if the policy works
CREATE OR REPLACE FUNCTION test_appointment_update_permission(appointment_id UUID, user_id UUID)
RETURNS BOOLEAN AS $$
DECLARE
  appointment_provider_id UUID;
  user_role TEXT;
BEGIN
  -- Get the provider_id of the appointment
  SELECT provider_id INTO appointment_provider_id 
  FROM appointments 
  ...
  RETURN (appointment_provider_id = user_id OR user_role = 'superadmin');
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
